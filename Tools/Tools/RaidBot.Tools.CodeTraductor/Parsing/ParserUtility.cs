...
namespace RaidBot.Tools.CodeTraductor.Parsing
{
    public class ParserUtility
    {
        ...
        {
            @namespace = @namespace.Replace("com.ankamagames.dofus.network.", "RaidBot.Protocol.Network.");
            string[] nameSpaceSplited = @namespace.Split('.');
            @namespace = "";
            for (int i = 0; i < nameSpaceSplited.Length - removeCount; i++)
            {
                string str = nameSpaceSplited[i];
                @namespace += str.Substring(0, 1).ToUpper() + str.Remove(0, 1) + ".";//remplace le premier character par une majuscule
        ...
        public static string[] GetImports(string[] imports)
        {
            ...
            retVal.Add("RaidBot.Protocol.Network.Messages");
            retVal.Add("RaidBot.Protocol.Network.Types");
            retVal.Add("RaidBot.Protocol.Network");
            retVal.Add("RaidBot.Protocol");
            retVal.Add("RaidBot.Common.IO");
